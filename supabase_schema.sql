...
-- 3.3 Predictions Table (Crop Recommendation, Yield, Risk)
CREATE TABLE IF NOT EXISTS public.predictions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  farm_id UUID REFERENCES public.farms(id) ON DELETE SET NULL,
  prediction_type TEXT NOT NULL, -- 'crop_recommendation', 'yield', 'climate_risk', 'fertilizer'
...
-- 3.4 AI Conversations Table (LLM Advisory Sessions)
...
-- 4.1 Automatically touch updated_at
...
-- 4.2 Auto-create or sync public.profiles when auth.users is created / updated
-- Works seamlessly with Google Sign-In and Email/Password Sign-Up
...
DECLARE
  ...
  assigned_role TEXT;
BEGIN
  -- Extract full name from raw metadata or email prefix
  ...
  -- Extract avatar or generate deterministic dicebear initial avatar
  ...
  -- Admin role assigned if explicit admin flag or email domain
  assigned_role := CASE
...
-- 4.3 Helper function to evaluate Admin status securely in RLS policies
...
-- ── 5.5 AI Conversations RLS Policies ──
...
-- 6.3 Storage Policies: Crop Scans
...
-- 6.4 Storage Policies: Farm Documents (Private)
